...
import com.thinkin_service.app.data.network.model.Payment;
...
//    TextView tvChange;
    ...
    public View initView(View view) {
        ...
        if (DATUM != null) {
            ...
            adapterPager = new SampleFragmentPagerAdapter(tabTitles, getFragmentManager(), getActivity());
    ...
    private void initView(@NonNull Datum datum) {
        ...
        SharedHelper.putKey(getContext(),"before",datum.getBefore_image());
        SharedHelper.putKey(getContext(),"after",datum.getAfter_image());
        SharedHelper.putKey(getContext(),"before_comm",datum.getBefore_comment());
         SharedHelper.putKey(getContext(),"after_comm",datum.getAfter_comment());
        ...
        try {
            if (payment != null) {
                if (payment.getFixed() > 0) {
                    ...
                    fixed.setText(String.format("%s", SharedHelper.getKey(getContext(), "currency") + (payment.getFixed())));
                } else llBaseFare.setVisibility(View.GONE);
                tax.setText(String.format("%s", SharedHelper.getKey(getContext(), "currency") + (payment.getTax())));
                total.setText(String.format("%s", SharedHelper.getKey(getContext(), "currency") + (payment.getTotal())));
