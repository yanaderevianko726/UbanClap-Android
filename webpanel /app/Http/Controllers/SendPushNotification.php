...
class SendPushNotification extends Controller {
    ...
    public function Dropped($request) {
        ...
        return $this->sendPushToUser($request->user_id, trans('api.push.dropped') . config('constants.currency') . $request->payment->total . ( $request->payment_mode == "CASH" ? "  CASH" : " via CARD" ));
    }
