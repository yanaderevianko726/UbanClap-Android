...
<div class="content-area py-1">
    <div class="container-fluid">
    	<div class="box box-block bg-white">
            ...
            <form  action="{{route('dispatcher.profile.update')}}" method="POST" enctype="multipart/form-data" role="form">
            	{{csrf_field()}}

				<div class="field">
					<label for="name" class="label">@lang('admin.name')</label>
					<div class="control">
						<input  class="input" type="text" value="{{ Auth::guard('dispatcher')->user()->name }}" name="name" required id="name" placeholder=" @lang('admin.name')">
					</div>
				</div>
				...
				<div class="field">
					...
					<div class="control">
						<input  class="input" type="email" required name="email" value="{{ isset(Auth::guard('dispatcher')->user()->email) ? Auth::guard('dispatcher')->user()->email : '' }}" id="email" placeholder="@lang('admin.email')">
					</div>
				</div>
				...
				<div class="field">
					...
					<div class="control">
						<input  class="input" type="text" value="{{ Auth::guard('dispatcher')->user()->mobile }}" name="mobile" required id="mobile" placeholder=" @lang('admin.mobile')">
					</div>
				</div>
