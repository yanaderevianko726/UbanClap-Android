...
<style>
    ...
    <?php if( !empty(Session::get('user')->language) && Session::get('user')->language=='ar'): ?> .arabic_rtl {
    ...
    jQuery(document).ready(function ($) {
        ...
        $('.group-select').on('change', function () {
            ...
            if (group) {
                ...
                Barryvdh\ TranslationManager\ Controller @getView ') ?>/' + $(this).val();
            } else {
                window.location.href = '<?= action('\
...
<div class="arabic_rtl" style="width: 80%; margin: auto;">
    ...
    <p>Warning, translations are not visible until they are exported back to the app/lang file, using publish button.
    </p>
    ...
    <p>
        ...
        <form class="form-inline form-import" method="POST"
            ...
            <select name="replace" class="input" <option value="0">Append new translations</option>
                <option value="1">Replace existing translations</option>
            </select>
            <button type="submit" class="button is-success" data-disable-with="Loading..">Import groups</button>
        </form>
        <form class="form-inline form-find" method="POST"
            ...
            <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
            <p></p>
            <button type="submit" class="button is-info" data-disable-with="Searching..">Find translations in
                files</button>
        </form>
        <?php endif; ?>
        <?php if(isset($group)) : ?>
        <form class="form-inline form-publish" method="POST"
            ...
            <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
            <button type="submit" class="button is-info" data-disable-with="Publishing..">Publish translations</button>
            <a href="<?= action('\Barryvdh\TranslationManager\Controller@getIndex') ?>"
                class="button is-default">Back</a>
        </form>
        <?php endif; ?>
    </p>
    <form role="form">
    ...
    <form action="<?= action('\Barryvdh\TranslationManager\Controller@postAdd', array($group)) ?>" method="POST"
        role="form">
        <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
        <textarea class="input" rows="3" name="keys"
            placeholder="Add 1 key per line, without the group prefix"></textarea>
        <p></p>
        <input type="submit" value="Add keys" class="button is-link">
    </form>
    <hr>
    ...
    <?php else: ?>
    <p>Choose a group to display the group translations. If no groups are visible, make sure you have run the migrations
...
<script type="text/javascript">
    ...
    <?php endif; ?>
</script><?php /**PATH /Users/apple/Documents/code/2020/thinkinservice2020/serviceadmin/resources/views/vendor/translation-manager/index.blade.php ENDPATH**/ ?>